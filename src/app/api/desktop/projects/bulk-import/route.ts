...
const bodySchema = desktopProjectSessionSchema.extend({
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await bulkImportUserProjectDirectories(session.userId, body.projects);
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: "Invalid desktop bulk import request.", details: error.flatten() },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unable to bulk import desktop projects.",
      },
      { status: 500 },
    );
  }
