...
import { createUserProjectDirectory, ProjectDirectoryError } from "@/lib/project-directories";
import { desktopProjectSessionSchema, projectDirectoryCreateSchema } from "@/lib/project-directory-schemas";

export const runtime = "nodejs";

const bodySchema = desktopProjectSessionSchema.merge(projectDirectoryCreateSchema);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await createUserProjectDirectory(session.userId, body);
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: "Invalid desktop project create request.", details: error.flatten() },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unable to create desktop project directory.",
      },
      { status: 500 },
    );
  }
